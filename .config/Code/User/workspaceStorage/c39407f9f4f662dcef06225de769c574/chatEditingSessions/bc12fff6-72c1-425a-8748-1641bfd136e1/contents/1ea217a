...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,
                 ...
                 vol_types_to_calc: Iterable[str] = ("raw", "gkyz", "skew", "kurtosis", "vol_zscore", "log_vol"),  ## rememebr to manage based on rocket or catboost
    ...
    def _add_tte_volatility_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        tte_features = ["time_to_exp1_hr", "tte_phase_sin", "tte_phase_cos"]
        
        for vol_feat in vol_features:
            if vol_feat in df.columns:
                # TTE × volatility level interactions
                if "time_to_exp1_hr" in df.columns:
                    df[f"{vol_feat}_x_tte"] = df[vol_feat] * df["time_to_exp1_hr"]
                    # Non-linear TTE effects
                    tte_normalized = df["time_to_exp1_hr"] / 168  # Normalize by week
                    df[f"{vol_feat}_x_tte_sq"] = df[vol_feat] * (tte_normalized ** 2)
                
                # Cyclical TTE effects  
        ...
        # Volatility term structure × TTE
        if {"vol_gkyz_3h", "vol_gkyz_24h", "time_to_exp1_hr"}.issubset(df.columns):
            vol_term_slope = df["vol_gkyz_24h"] - df["vol_gkyz_3h"]
            df["vol_term_x_tte"] = vol_term_slope * df["time_to_exp1_hr"]

...
class VolatilityRegimeEngineer(BaseEstimator, TransformerMixin):
    def __init__(self, 
                 lookback_window: int = 24, # Use 24h for responsiveness
                 seasonal_window: int = 720,
                 forward_window: int = 24,
                 
                 # --- NEW INPUT LOGIC ---
                 # Both of these are now "Standard Deviations of the FORWARD WINDOW".
                 # e.g., "I want a move equal to 1.2 Daily Sigmas"
                 trend_std: float = 1.2,
                 
                 # "I want a move equal to 1.5 Daily Sigmas... but done in 3 hours"
                 # This guarantees Jump > Trend if you set this > trend_std
                 jump_std: float = 1.5,
    ...
    def fit(self, X: pd.DataFrame, y=None):
        # ... (Fit method stays exactly the same) ...
        required_cols = ['c', 'h', 'l']
        if not all(col in X.columns for col in required_cols):
    ...
    # ... (_build_seasonal_vol_lookup stays same) ...
    # ... (_calculate_lookback_box stays same) ...
    ...
    def _assign_regime_labels(self, X: pd.DataFrame, box_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # This converts a "Short Window Z-Score" into "Forward Window Units"
        # Example: sqrt(24 / 3) = 2.83
        # We must MULTIPLY the Jump Threshold by this to make it apples-to-apples
        time_scaling_factor = np.sqrt(self.forward_window / self.jump_speed_window)
        
        # The Actual Z-Score threshold required for the short window
        # If input jump_std=1.5 (Daily), required 3h Z = 1.5 * 2.83 = 4.24
        ...
        for idx in X.index:
            ...
            for i in range(len(fwd_indices) - self.jump_speed_window + 1):
                ...
                # Standard 3h Z-Score calculation
            ...
            # --- CLASSIFICATION (Using Normalized Logic) ---
            
            # Use the SCALED Threshold here
            if max_jump_z > REAL_JUMP_THRESHOLD:
                results.at[idx, 'regime_label'] = 2
                continue
            
            # Standard Trend Threshold
    ...
    # ... (Rest of transform/etc remains same) ...
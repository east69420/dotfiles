...
class VolatilityRegimeEngineer(BaseEstimator, TransformerMixin):
    def __init__(self, 
                 ...
                 trend_min_efficiency: float = 0.25, # 0.0 to 1.0
                 trend_min_r2: float = 0.6           # 0.0 to 1.0 (Linear fit)
                 ):
    ...
    def _assign_regime_labels(self, X: pd.DataFrame, box_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Pre-calc single-period returns for Efficiency Ratio
        ...
        for idx in X.index:
            ...
            # --- HARDENED TREND LOGIC ---
            if fwd_z_score > self.trend_std:
                
                # 1. Efficiency Check (Noise Filter)
                # Sum of absolute hourly moves in the window
                total_path_travel = abs_diffs.loc[fwd_indices].sum()
                net_move = abs(np.log(final_close / ref_price))
                
                # Avoid div/0
                efficiency = net_move / total_path_travel if total_path_travel > 1e-9 else 0.0
                
                # 2. Linearity Check (Correlation)
                # Is the price roughly forming a straight line?
                fwd_prices = safe_close.loc[fwd_indices].values
                if len(fwd_prices) > 2:
                    # Correlation of Prices vs [0, 1, 2, 3...]
                ...
                is_efficient = efficiency >= self.trend_min_efficiency
                is_linear = r2 >= self.trend_min_r2
                
                if is_efficient and is_linear:

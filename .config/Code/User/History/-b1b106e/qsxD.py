...
def run_baseline_for_target(target, percentile, X_train, y_train, X_val, y_val, model_params):
    ...
    # For regression mode (percentile == -1), use mean as baseline
    if percentile == -1:
        baseline_pred = np.full_like(y_val, np.nanmean(y_train), dtype=float)
    else:
        q = percentile
...
for target, plan_entry in stage1_feature_plan.items():
    ...
    # Skip only if truly empty (not if it contains regression sentinel -1)
    if not percentiles or (len(percentiles) == 1 and percentiles[0] is True):

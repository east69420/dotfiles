...
class VolatilityRegimeEngineer(BaseEstimator, TransformerMixin):
    def __init__(self, 
        ...
        self.global_vol_median = None  # NEW: Store global average
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        # 2. NEW: Learn the Global Average Volatility (The Scalar)
        # This gives us the "Unit" to multiply the ratio by.
    ...
    def _calculate_lookback_box(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. Calculate Raw Rolling Volatility (Current Conditions)
        safe_open = X['o'].replace(0, np.nan) if 'o' in X.columns else X['c'].replace(0, np.nan)
        safe_high = X['h'].replace(0, np.nan)
        safe_low = X['l'].replace(0, np.nan)
        safe_close = X['c'].replace(0, np.nan)
        ...
        # 2. Get Seasonal Norm for every timestamp
        seasonal_vol_series = pd.Series(index=X.index, dtype=float)
        
        # (Optimized lookup using map)
        # Create a tuple series to map against
        ...
        # Logic: (Raw / Seasonal) * Global_Median
        # If Raw=0.002 (Sunday) and Seasonal=0.002 (Sunday), Ratio=1.0.
        # Adjusted = 1.0 * 0.01 (Global) = 0.01.
        # This treats Sunday as if it were a normal Monday.
        
        deseasonalized_ratio = raw_vol / seasonal_vol_series
        adjusted_vol = deseasonalized_ratio * self.global_vol_median
        
        df['box_std'] = adjusted_vol # This is now in correct units (e.g., 0.01)
        df['raw_vol'] = raw_vol
    ...
    def _assign_regime_labels(self, X: pd.DataFrame, box_df: pd.DataFrame) -> pd.DataFrame:
        # ... (This method remains exactly the same as previous version) ...
        # ... But now 'box_std' is the correct scale (approx 0.01 instead of 1.0) ...
        ...
        for idx in X.index:
            ...
            final_close = X.iloc[fwd_end_pos-1]['c'] # Get actual close at end of window
            ...
            # --- Z-SCORE CALCULATION ---
            # Now dividing Percentage by Percentage. Scale is correct.
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        output = results.copy()
        output['box_std_deseasonalized'] = box_df['box_std'] # The Adjusted Vol
